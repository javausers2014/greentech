...
public class TaskConfigDAOHibernate extends HibernateDaoSupport implements TaskConfigDAO {
    ...
    public IArcTaskConfig getArcTaskConfig(Long taskConfigId) {
       // Long jobIdlong = Long.valueOf(jobId);
        ...
        return (IArcTaskConfig) getHibernateTemplate().get(ArcTaskConfig.class, taskConfigId);
    }
     ...
     * @return DOCUMENT ME!
     */
    public List findArcTaskConfig(final IArcTaskConfig taskConfig) {
        List jobs=(List) getHibernateTemplate().execute(new HibernateCallback() {
                    public Object doInHibernate(Session ses)
                        ...
                        List results = ses.createCriteria(ArcTaskConfig.class).add(example)
                                          .list();
    ...
    /**
     * 
     * @see com.Arctechnologies.scheduler.persistence.TaskConfigDAO#insertArcTaskConfig(com.ArcTaskConfig.scheduler.model.ArcTaskConfig)
     * @param taskConfig
     */
    ...
    /**
     * 
     * @see com.Arctechnologies.scheduler.persistence.TaskConfigDAO#removeArcTaskConfig(com.ArcTaskConfig.scheduler.model.ArcTaskConfig)
     * @param taskConfig
     */
     ...
     * @param job DOCUMENT ME!
     */
    public void updateArcTaskConfig(IArcTaskConfig job) {
        if (null != job) {
     ...
     * @return ArcJob
     */
    public ArcTaskConfig getArcTaskConfig(String jobSheduledName) {
        ...
        if (jobSheduledName!=null && jobSheduledName.trim().length()>0) {
            ...
            return (ArcTaskConfig)rs.iterator().next();
        }else{
            return null;
        }        
    }
